...
def describe_asc():
    def describe_read_asc_file_reader():
        def when_no_columns():
            results = list(read_asc_file("./tests/fixtures/sample.asc"))
        ...
        def when_columns():
            results = list(
                read_asc_file(
                    ...
                    columns=["code", "name", "abbrev", "none", "none2"],
                )
            )
            expect(len(results)) == 3
        ...
        def when_bad_columns():
            with pytest.raises(ValueError):
                results = list(
                    read_asc_file(
                        "./tests/fixtures/sample.asc", columns=["code", "name"]
                    )
                )
        ...
        def when_utf8_and_comma():
            results = list(
                read_asc_file(
                    "./tests/fixtures/sample_utf8_comma.asc",
                    encoding="utf-8",
                    separator=",",
                    columns=["code", "name", "abbrev", "none", "none2"],
                )
